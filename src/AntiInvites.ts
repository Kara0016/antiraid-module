...
import { inviteRegex, DefaultAntiInvitesOptions } from "./util/Constants";

const log = console.log;


export class AntiInvites extends EventEmitter {
  client: any;
  regex: RegExp;
  _cache: any[];
  options: any;
      ...
      message.channel
        ...
        .catch((e) => {
          if (this.options.verbose)
            console.error(
              `AntiRaid (sendMessage#sendMissingPermMessage): ${e.message}`
            );
        });
    }
  }
 ...
 */
  async punish(message) {
    ...
    let level;
    const cases = this._cache.filter(
      (_case) =>
        _case.ID === message.author.id && _case.guild === message.guild.id
    );
    ...
    switch (level) {
      ...
      case 1: // mute
        if (!member.manageable) {
          if (this.options.verbose)
            log(
              `AntiRaid (punish#userNotManageable): ${member.user.tag} (ID: ${member.user.id}) could not be manageable, insufficient permissions`
            );
        } else {
          const muteRole = message.guild.roles.cache.find(
            (r) =>
              r.name.toLowerCase() === this.options.muteRoleName.toLowerCase()
          );
          if (!muteRole) {
            if (this.options.verbose)
              log(
                "AntiRaid (punish#muteRoleNotFound) could not found muted role named " +
      ...
      case 2: // kick
        if (!member.kickable) {
          if (this.options.verbose)
            log(
              `AntiRaid (punish#userNotKickable): ${member.user.tag} (ID: ${member.user.id}) could not be kicked, insufficient permissions`
            );
        } else {
          member
            .kick("Anti-Invites")
      ...
      case 3: // ban
        if (!member.bannable) {
          if (this.options.verbose)
            log(
              `AntiRaid (punish#userNotBannable): ${member.user.tag} (ID: ${member.user.id}) could not be banned, insufficient permissions`
            );
        } else {
          member
            .ban({
  ...
  async message(message) {
    ...
    if (
      this.options.ignoredRoles.some((r) => message.member.roles.cache.has(r))
    )
      return false;
    if (
