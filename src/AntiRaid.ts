...
export class AntiRaid extends EventEmitter {
  ...
  addCase(member, event): void {
    ...
    setTimeout(
      () => {
        this._cases = this._cases.filter((c: any) => c.key !== key);
      },
  ...
  checkCase(member, event) {
    const _case = this._cases.filter(
      (c) =>
        c.ID === member.id && c.guild === member.guild.id && event === c.event
    );
        ...
        if (!member.manageable) {
          ...
          for (const role of roles.array()) {
            if (
              role.permissions
                .toArray()
                .find((p) => blacklistedPermissions.includes(p))
            ) {
              if (!role.editable) {
                if (this.options.verbose)
                  log(
                    `AntiRaid (punish#roleNotManageable): ${member.user.tag} (ID: ${member.user.id}) could not be manageable, insufficient permissions`
                  );
              } else {
                if (member.user.bot) {
                  role
